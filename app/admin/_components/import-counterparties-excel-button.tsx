...
type CounterpartyImportRowResult = {
  rowNumber: number;
...
type CounterpartyImportValidationResult = {
...
type ImportCounterpartiesExcelButtonProps = {
  label?: string;
};

export function ImportCounterpartiesExcelButton({
  label = "Импорт из Excel",
}: ImportCounterpartiesExcelButtonProps) {
  ...
  const [validation, setValidation] = useState<CounterpartyImportValidationResult | null>(
    null,
  );
  ...
  async function handleTemplateDownload() {
    try {
      const response = await fetch("/api/admin/counterparties/import/template");
      if (!response.ok) {
      ...
      const filename =
        ...
        "counterparties-import-template.xlsx";
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/counterparties/import/validate", {
        method: "POST",
        body: formData,
      });
      const data = (await response.json()) as CounterpartyImportValidationResult & {
        error?: string;
      };
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const response = await fetch("/api/admin/counterparties/import", {
        method: "POST",
      ...
      const message =
        error instanceof Error && error.message !== "import_failed"
          ? error.message
          : "Не удалось импортировать контрагентов";
      toast.error(message);
    } finally {
      setImporting(false);
    }
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="flex max-h-[min(90vh,720px)] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
          <DialogHeader className="shrink-0 px-6 pt-6">
            <DialogTitle>Импорт контрагентов из Excel</DialogTitle>
            <DialogDescription>
              Скачайте шаблон, заполните лист «Контрагенты» и загрузите файл. Ошибки
